    ...
    st.markdown(
        ...
        <style>
          ...
          .stButton>button {{ width: 100%; border: 2px solid rgba(0,0,0,0.06);
            ...
            transition: background .12s ease, transform .08s ease; border-radius: 14px; }}
          ...
          /* Overlay */
          .overlay {{ position: fixed; inset: 0; background: rgba(0,0,0,0.28);
...
def render_bg_animation():
    ...
    <div class="bg-anim" style="position:fixed; inset:0; pointer-events:none; z-index:0; overflow:hidden;">
...
# Welcome overlay (with explicit Close button control below the card to ensure it's clickable)
if st.session_state.show_welcome_overlay and st.session_state.phase == "welcome":
    st.markdown(
        ...
        "<div style='font-weight:900; font-size:18px;'>Welcome</div>"
        "<div style='font-size:14px; opacity:.75;'>Click Close to begin</div>"
        "</div>"
        ...
        "</div>"
        "<div style='margin-top:8px; font-size:13px;'>In the U.S., call <b>911</b> for emergencies, or call/text <b>988</b> for mental health crises.</div>"
        "</div></div>",
        unsafe_allow_html=True,
    )
    st.write("")  # spacing
